...
from utils.linkedin_data_loader import build_community_directory_data, load_li_institution_tags
...
st.markdown(
    ...
    <div class="li-banner">
        ...
        <p>Alumni &amp; Trainee Profiles · Hotchkiss Brain Institute</p>
    </div>
    """,
...
# ── Sidebar filters ───────────────────────────────────────────────────────────
_FILTER_KEYS = ["comm_search", "comm_area", "comm_employer", "comm_location", "comm_edu", "comm_institution"]
...
if st.session_state.pop("_comm_reset", False):
    for _k in _FILTER_KEYS:
        st.session_state[_k] = [] if _k == "comm_institution" else ""
...
with st.sidebar:
    ...
    filter_area = st.text_input("Research Area / Area of Interest", placeholder="e.g. neuroscience", key="comm_area")
...
if search_query:
    ...
    mask = (
        ...
        | filtered["current_title"].str.lower().str.contains(q, na=False)
        | filtered["current_company"].str.lower().str.contains(q, na=False)
        | filtered["research_tags_list"].apply(
            lambda tags: any(q in t.lower() for t in tags)
        )
...
filtered = filtered.sort_values("full_name").reset_index(drop=True)
...
_fhash = hash((search_query, filter_area, filter_employer, filter_location, filter_edu, tuple(sorted(filter_institution)), per_page_choice))
