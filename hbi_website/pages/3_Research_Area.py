...
if not area:
    ...
    st.markdown(
        ...
        '<h1>Explore Research Areas</h1>'
        '<p style="font-size:1.1rem;opacity:0.9;">'
        'Browse all research areas · Discover the research interests of our HBI Members and Community</p>'
        '</div>',
    ...
    _sort_map = {
        ...
        "Research Area  A → Z": (["area"],                   [True]),
        "Research Area  Z → A": (["area"],                   [False]),
    }
    ...
    st.caption(f"{len(_topics)} research area{'s' if len(_topics) != 1 else ''}{_filter_label}")
    ...
    for _, _row in _topics.iterrows():
        ...
        cards_html += (
            ...
            f'<div class="topic-card-name">{_esc(_tag)}</div>'
...
if not community_matches.empty:
    ...
    st.caption(f"{len(community_matches)} community profiles with this research area")
    ...
    if view_mode == "Grid":
        ...
        for _, row in community_matches.iterrows():
            ...
            if not isinstance(tags, list):
                tags = []
            comm_cards.append(render_community_card_html(fname, headline, employer, location, tags, profile_id=pid))
        ...
        for _, row in community_matches.iterrows():
            ...
            if not isinstance(tags, list):
                tags = []
            comm_rows.append(render_community_list_row_html(fname, headline, employer, location, tags, profile_id=pid))
