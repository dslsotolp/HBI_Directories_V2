...
if not org:
    ...
    st.markdown("---")
    st.caption("Data sourced from UCalgary Profiles & LinkedIn · Hotchkiss Brain Institute")
    st.stop()
    ...
    if view_mode == "Grid":
        ...
        for _, row in filtered_community.iterrows():
            ...
            if not isinstance(tags, list):
                tags = []
            comm_cards.append(
                render_community_card_html(fname, ctitle, ccompany, location, tags, profile_id=pid)
            )
        ...
        for _, row in filtered_community.iterrows():
            ...
            if not isinstance(tags, list):
                tags = []
            comm_rows.append(
                render_community_list_row_html(fname, ctitle, ccompany, location, tags, profile_id=pid)
            )
