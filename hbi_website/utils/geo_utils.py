...
geo_utils.py
============
Static geocoding helpers for mapping community profile locations.

Design: no external API calls — a curated dict covers all distinct city values
seen in the LinkedIn dataset (~30 cities) plus common Canadian university cities
reachable via the HBI member education institution data.
"""
...
def geocode_location(location: Optional[str]) -> Optional[tuple[float, float]]:
    """
    Return (lat, lon) for a LinkedIn location string, or None if unresolvable.

    Lookup strategy (most-specific first):
    1. Full string match (lowercase) against CITY_COORDS
    2. "city, region, country" (all three parts)
    3. "city, country" (skip region)
    4. "city, region"
    5. "city" alone
    """
    ...
    # Strip LinkedIn remote-work suffixes ("Calgary, Canada · On-site")
    loc = re.sub(r"\s*[·•]\s*(?:on-site|remote|hybrid).*$", "", str(location).strip(), flags=re.I)
    loc = loc.strip().lower()

    # Direct full-string lookup
    if loc in CITY_COORDS:
        return CITY_COORDS[loc]

    # If no commas (e.g. "Calgary Canada"), try treating each space-separated
    # word as a potential city and fall through to city-alone lookup
    if "," not in loc:
        words = loc.split()
        city_guess = words[0] if words else ""
        if city_guess and city_guess in CITY_COORDS:
            return CITY_COORDS[city_guess]

    city, region, country = parse_location_string(loc)
    ...
    candidates = [
        ...
        f"{city}, {region}",
        city,
        f"{region}, {country}" if not city else "",
        country if not city else "",
    ]
    for candidate in candidates:
        candidate = candidate.strip(", ")
        if candidate and candidate in CITY_COORDS:
