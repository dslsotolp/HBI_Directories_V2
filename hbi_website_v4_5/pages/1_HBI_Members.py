    ...
    if page_data.empty:
        ...
        if view_mode == "Grid":
            ...
            for _, row in page_data.iterrows():
                ...
                cards.append(render_card_html(nm, title, dept, areas, member_id=mid, photo_url=_photo))
            ...
            for _, row in page_data.iterrows():
                ...
                _pp = row.get("photo_path", "")
                _photo = f"/app/static/images/member/{_pp}" if _pp and pd.notna(_pp) else None
                rows_html.append(render_list_row_html(nm, title, dept, areas, member_id=mid, photo_url=_photo))
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not mem_acts.empty:
        with st.container(key="hbi_panel_activities"):
            profile_panel_header(
                "Activities",
                "Activities, awards, and recognitions are drawn from the "
                "researcher’s public UCalgary Research profile.",
            )
            for cat in mem_acts["category"].dropna().unique():
                cat_rows = mem_acts[mem_acts["category"] == cat]
                category_label = str(cat).replace("_", " ").title()
