...
def render_profile_background_html(
    ...
    for row in education:
        ...
        if parts:
            education_items.append(f'<li>{_esc(", ".join(parts))}</li>')
    if education_items:
        sections.append(
...
def render_profile_at_a_glance_html(
    ...
    valid_recognitions = sorted(
        (
            recognition
            for recognition in (recognitions or [])
            if str(recognition.get("title") or "").strip()
        ),
    ...
    keyword_count = _format_profile_metric(summary.get("unique_keyword_count"))
    ...
    if keyword_count:
        metrics.append((keyword_count, "Research keywords"))
    ...
    if valid_recognitions:
        metrics.append((f"{len(valid_recognitions):,}", "Recognitions listed"))
    ...
    if metrics:
        ...
        metrics_html = (
            ...
            '<span class="hbi-profile-help-text" role="tooltip">'
            "Publication totals, research-keyword totals, and publication span "
            "are calculated from the publication records connected to this "
            "profile. The recognition count comes from awards listed on the "
            "researcher’s public UCalgary Research profile.</span></span></div>"
    ...
    if valid_recognitions:
        items = "".join(
            ...
            + f'<strong>{_esc(recognition.get("title"))}</strong></div>'
        ...
        recognitions_html = (
            ...
            "researcher’s public UCalgary Research profile. The three most "
            "recent dated entries are shown.</span></span></div>"
...
def render_research_tags(areas: list, max_show: int = 6, clickable: bool = True) -> str:
    ...
    for area in areas[:max_show]:
        ...
        if clickable:
            href = f'/Areas_of_Expertise?area={quote(str(area))}'
...
def render_list_row_html(name: str, title: str, department: str, areas: list,
                         ...
                         context_text: str = "", tag_version: int = 1) -> str:
...
def render_community_research_tags(
    ...
    for tag in tags[:max_show]:
        display_tag = format_research_label(tag)
        if clickable:
            href = f'/Areas_of_Expertise?area={quote(str(tag))}'
...
def inject_custom_css():
    ...
    .st-key-hbi_panel_activities h5,
    ...
    .st-key-hbi_panel_activities [data-testid="stCaptionContainer"] p,
    ...
    .st-key-hbi_panel_activities [data-testid="stWidgetLabel"] p,
    ...
    .st-key-hbi_panel_activities .stMarkdown a:not([class]),
    ...
    .st-key-hbi_panel_activities .stMarkdown a:not([class]):hover,
...
def render_disclaimer():
    st.markdown("---")
    st.caption(
        "Data sourced from UCalgary Profiles, LinkedIn, and publicly indexed "
