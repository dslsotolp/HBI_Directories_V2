...
def _exp_text_for_profile(profile_exp: pd.DataFrame, min_year: int) -> str:
    """Concatenated lowercase descriptions of experience entries ending >= min_year."""
    recent = profile_exp[profile_exp["end_date"].apply(_parse_exp_year) >= min_year]
    return " ".join(
        str(d).lower()
...
def _recent_pubs_text_for_profile(
    ...
    """Concatenated lowercase title + description of publications published >= min_year.
    ...
    recent = profile_pubs[
        profile_pubs["pub_date"].apply(_parse_exp_year) >= min_year
...
def get_all_profiles_recent_tags(min_year: int = 2016) -> dict:
    ...
    if _pub_cols and "pub_date" in pubs_df.columns:
        ...
        recent_pubs = pubs_copy[pubs_copy["_year"] >= min_year]
    ...
    exp_df["_year"] = exp_df["end_date"].apply(_parse_exp_year)
    recent_exp = exp_df[exp_df["_year"] >= min_year]
    recent_texts: dict = {
        pid: " ".join(
            str(d).lower()
...
def get_filtered_profile_tags_recent(
    ...
    pubs_text = _recent_pubs_text_for_profile(profile_id, load_li_publications(), min_year)
    profile_exp = load_li_experience()
    profile_exp = profile_exp[profile_exp["profile_id"] == profile_id].copy()
    exp_text = _exp_text_for_profile(profile_exp, min_year)
...
def build_community_directory_data() -> pd.DataFrame:
    ...
    # (1) mutual-connections strings leaked into location column
    # (2) job-title strings captured as location (near-duplicate of current_title)
    _conn_re = re.compile(r'\d+\s+(?:mutual\s+)?connection', re.IGNORECASE)
